...
import { MessageFactory, StatePropertyAccessor, TeamsInfo, TurnContext } from 'botbuilder';
...
export class TeamsInfoDialog extends ComponentDialog {
  constructor() {
    ...
    this.addDialog(new TextPrompt('TextPrompt')).addDialog(
      new WaterfallDialog(TEAMS_INFO_WATERFALL_DIALOG_ID, [this.introStep.bind(this), this.actStep.bind(this), this.finalStep.bind(this)])
    );
  ...
  private async finalStep(stepContext: WaterfallStepContext): Promise<DialogTurnResult> {
    await stepContext.context.sendActivity(
      "Now it's time to add more functionality to your bot, so head over to the [docs](https://aka.ms/yoTeams) and start building"
    );
